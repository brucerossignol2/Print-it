...
const dotsContainer = document.getElementById("dotsContainer");	// Conteneur des points
...
function updateSelectedIndex(selectedIndex) {
	...
	dotsContainer.innerHTML = "";
  	console.log("Le conteneur 'dotsContainer' a été effacé.");
...
for (let i = 0; i < nombreDeSlides; i++) {
  ...
  // Ajouter la classe "dot_selected" au point sélectionné
	...
	dotsContainer.appendChild(dot); // Ajouter chaque point au conteneur
}}
// écoute les clics sur les flêches
flecheGauche.addEventListener("click", () => {
	console.log(`Flêche de gauche clickée. selectedIndex ${selectedIndex}`);
...
flecheDroite.addEventListener("click", () => {
	console.log(`Flêche de droite clickée. selectedIndex ${selectedIndex}`);
	selectedIndex = selectedIndex +1;
